...
[`scripts/build_mapped_poc_snapshot.py`](../scripts/build_mapped_poc_snapshot.py) is the deterministic ingestion and derivation boundary. It selects one `MS_G` record per acquisition, globally deduplicates by `capture_uuid` while retaining the original mission copy, includes the two genuine mission-004 captures omitted by the earlier workbook generator, and orders source records by mission index then capture number. `TREE-0001...TREE-0112` supplies the workbook's static Ganoderma risk score and health classification in that same order. The generator also creates the minimum repository-owned evidence needed by the UI: one size-bounded natural-colour WebP derivative for every observation displayed as infected or suspected. Source checksums, evidence checksums, mission names, status provenance, geofence kind, and record origin make regeneration inspectable.

The revised survey contract contains 137 records across five stable areas, with counts `52 / 13 / 17 / 53 / 2`:

- `MPOC-SURVEY-001` has 27 source-backed records from `DJI_202606200943_001_DJI-SmartFarm-Web`. Their displayed status is **Infected** because the user explicitly designated every acquisition in that mission as infected; this status is independent of the workbook's modelled risk value. The area also has the fixed layout-only batch `TREE-0113...TREE-0137`, displayed as **Infected** with deterministic modelled scores from 66 through 95, so its 8x8 layout contains the requested 52 trees. These 25 records have no capture UUID, capture time, source mission, or evidence image; their operational coordinates exist only after the browser-local marker workflow succeeds.
...
- The combined display-status totals are 52 infected, 20 suspected, and 65 healthy. The original workbook classification and modelled score remain separately inspectable rather than being rewritten to manufacture the user-designated source-record infection state.

The snapshot is the only runtime data source for Mapped POC. It contains stable identifiers, nullable capture metadata, camera exposure coordinates, modelled values, display status and its source, area geofences, evidence-relative paths, and provenance. It contains no absolute `C:` or `G:` path and the browser performs no runtime reads from either drive. The workbook and DJI mission inventory remain external, read-only generator inputs. The 47 optimized evidence derivatives are stored under [`assets/mapped-poc/evidence`](../assets/mapped-poc/evidence); original captures are neither copied nor served.

Each survey area stores a deterministic convex hull of its source camera exposure coordinates. Areas 002-005 continue to render that hull through Leaflet as a **camera-footprint geofence**. Area 001 retains the hull as source provenance, but its effective survey-area geofence is the editable Google Maps rectangle documented below. Neither representation is a legal farm boundary, surveyed palm perimeter, acreage, or ownership evidence. Overview and survey-area maps render infected and suspected record pins. Mapped POC Tree Detail no longer renders a per-tree camera-position map or evidence image. Source-backed labels continue to say **camera observation** and **modelled Ganoderma risk**, because exposure coordinates are not palm-base surveys and model outputs are not field or laboratory diagnoses. A record has only one model score, so no simulated recent-risk trend is shown.

The existing visual language and 8x8 survey layout remain unchanged. Area 001 fills 52 cells and leaves 12 black disabled cells; the other areas retain capture-sequence order and black unused cells. Long capture UUIDs, mission names, metadata values, legend rows, and responsive detail columns use shrinkable grid tracks and explicit wrapping so they cannot force content across the page boundary.

The implementation gate in [`tests/static-check.mjs`](../tests/static-check.mjs) verifies the `137`-record and `52 / 13 / 17 / 53 / 2` area contract; `52 / 20 / 65` display-status totals; unique IDs and source capture UUIDs; the source-backed versus layout-only nullability contract; mission-004 corrections and duplicate exclusion; finite source coordinates; five camera-footprint convex hulls; 47 local evidence derivatives and their checksums; conditional pins/images; healthy-detail omission; overflow CSS; no simulated trend or runtime drive dependency; removal of the four synthetic Mapped POC farms; and preservation of the other companies and selector.

On 2026-09-10, independent `npm test`, `npm run build`, and `git diff --check` runs were green; Git reported line-ending warnings only. Manual browser acceptance verified the four-company selector, all five area geofences and counts, Area 001's 52/64 layout, infected and suspected pins/images, healthy-page evidence omission, first/last record navigation, exact risk and provenance copy, and containment of long metadata at a narrow viewport.

## Decision: Survey Area 001 Google Satellite geofence editor

Only `MPOC-SURVEY-001` uses the Google Maps JavaScript API. The integration loads lazily when Area 001 opens, selects Satellite imagery, and renders its 27 user-designated infected camera-position pins. Areas 002-005 retain their existing Leaflet maps and deterministic camera-footprint hulls; the Google integration does not change their data or rendering path. Map failure is non-blocking, so Area 001's 8x8 tree grid and record navigation remain available.
...
The editor uses the native `google.maps.Rectangle` with `editable` and `draggable` enabled only during explicit edit mode. It does not load or depend on the discontinued Google Maps Drawing Library. The shared browser-local document is version `4` and retains `geofenceOverrides`, keyed by `AREA_ONE_ID`, while adding the Area 001 tree-position collection documented below; migration preserves prior valid demo state. Only a finite, non-zero rectangle with `south < north`, `west < east`, and coordinates inside the configured Andhra Pradesh limits is accepted. Invalid or unavailable storage leaves the deterministic or last valid in-memory boundary authoritative.
...
Area 001 adds one native select control containing only its 27 infected, source-backed tree records. Selection state is deliberately transient and stores the exact Tree ID only; it is not added to browser-local persistence or the data snapshot. Selecting a tree pans and zooms the Google map to its camera position. The corresponding marker becomes enlarged, outlined, and check-marked, while adjacent text identifies the selected Tree ID so selection is not conveyed by colour or marker shape alone. **Open selected tree** and direct marker clicks both resolve through that exact ID and open the existing route for the matching tree record. Areas 002-005 receive no selector or selection-state changes. The intentionally small scope adds no search, grouping, bulk selection, or new persistence contract.

## Decision: Survey Area 001 fixed-batch Add Marker workflow

**Add marker** is available only after the user explicitly saves a valid Area 001 geofence, and it handles exactly the fixed `TREE-0113...TREE-0137` batch. Map clicks assign those 25 IDs sequentially inside the saved rectangle; each record retains its deterministic 66-95 modelled risk score. Draft placement, undo, and cancel remain transient. **Save markers** accepts only the complete 25-position batch with finite, unique coordinates inside the effective saved bounds and persists that full mapping atomically in the version-4 browser-local document. An incomplete or invalid batch, or a failed storage write, does not expose a partially saved placement.

After a successful save, Area 001 renders its original 27 source-backed pins plus the 25 added positions, for 52 infected pins. A later geofence save or reset is rejected when the proposed bounds would exclude any saved position, preventing a boundary/marker conflict. Areas 002-005 do not read or mutate this state. Tree-detail behavior for the newly positioned batch is deliberately deferred; this increment establishes placement and survey-map visibility only. The concise implementation and invariants are pinned in [`index.html`](../index.html) and [`tests/static-check.mjs`](../tests/static-check.mjs); no additional marker classes, arbitrary tree creation, or backend contract is introduced.

## Decision: simplified active Tree Detail surfaces

All active Tree Detail routes omit image galleries. Mapped POC Tree Detail additionally omits the per-tree camera-position map; the stale Leaflet tree-detail initialization path was removed with that surface. Tree Detail retains its Ganoderma indicators, camera and record metadata, observation history, recommendation, and status-interpretation content. This presentation change does not alter survey-area maps, geofences, marker placement, marker selection, or exact-tree navigation.

On 2026-09-10, the temporary key successfully loaded the live Google Satellite map for Area 001. Independent `npm test`, `npm run build`, and `git diff --check` gates were green. Verification covered lazy Area 001-only loading, all-role edit access, the exact default bounds, 27 source-backed infected pins, complete 25-marker placement and 52-pin rendering after save, live coordinate updates, geofence and marker save/cancel/reset behavior, version-4 migration and reload persistence, invalid-bound and partial-batch rejection, specific load-error fallbacks, unchanged Leaflet behavior for Areas 002-005, and continued tree-grid access when Google Maps is unavailable.
...
- AP portfolio coordinates, operational health readings, tree histories, and Ganoderma indicators remain simulated deterministic demo values. Mapped POC is the documented exception: its source-backed coordinates are DJI camera exposure positions and its workbook scores are deterministic/modelled POC values; neither is a surveyed palm-base position or diagnosis.
