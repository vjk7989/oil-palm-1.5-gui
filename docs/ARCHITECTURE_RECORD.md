...
- `MPOC-SURVEY-003` activates 27 immutable **source-copy** records from `TREE-0201...TREE-0227` and zero through 37 optional records from `TREE-0228...TREE-0264`.

The fresh portfolio therefore has 89 fixed Infected trees; the maximum is 192 active trees when all optional capacity is filled. Each area still renders at most 64 active records in its existing 8x8 layout, with unused cells black and disabled. Optional Red/Infected, Yellow/Suspected, and Green/Healthy records receive deterministic modelled scores in the corresponding 66-95, 35-65, and 10-34 bands. Marker-selected status is user-designated and is not a field or laboratory diagnosis.

The snapshot is the immutable data and capacity source. Versioned browser-local state supplies only independently saved geofences and optional marker positions. Snapshot records contain stable identifiers, nullable capture metadata, camera exposure or operational display coordinates, modelled values, display status and its source, geofence defaults, evidence-relative paths, and provenance. The browser performs no runtime reads from source drives. Source inputs remain external and read-only; only optimized, repository-owned evidence files are served.
...
The shared browser-local document is version `8`. Migration preserves valid Area 001 and Area 002 geofence/marker state without reinterpretation and initializes isolated `areaThreeTreePositions` plus `geofenceOverrides["MPOC-SURVEY-003"]`. Every editor reads and writes only its own state branch. Invalid or unavailable storage leaves the deterministic or last valid in-memory data authoritative.
...
## Decision: Survey Area 003 explicit source-copy

Area 003 is an independent editable workspace at the same location as Area 001, not evidence of 27 additional captures. Its fixed `TREE-0201...TREE-0227` records copy Area 001 coordinates, capture metadata, deterministic infected scores, statuses, and image references one for one. Each record declares `copiedFromAreaId: "MPOC-SURVEY-001"` and the corresponding `copiedFromTreeId`. It retains the real capture UUID. Duplicate capture UUIDs are permitted only for these explicitly paired Area 001-to-Area 003 source-copy records; Tree IDs remain globally unique.

Area 003 shares Area 001 image files instead of creating duplicate media. Infected or Suspected Area 003 additions resolve the nearest shared allowed Area 001 image and retain the non-exact-tree disclosure; Healthy additions show no image. The 20 derivatives that belonged only to the retired survey areas were deleted.

Area 003 has its own saved rectangle, optional-marker collection, editing mode, selected tree, and transient draft. Its fixed source-copy markers are immutable and red. Optional IDs `TREE-0228...TREE-0264` support the same add/remove/undo/cancel/atomic-save behavior and score/status bands as Area 001 without reading or mutating Area 001 state.
...
The version-8 three-survey replacement passed the independent static contract, production build, and diff-whitespace gates. Coverage pins the exact area order, removal of retired runtime records and assets, all 27 Area 003 source-copy pairings, the narrow duplicate-UUID exception, shared images, independent state migration/editors, combined Overview pin actions, fresh 89-tree and maximum 192-tree totals, and preservation of the other three companies.
...
- AP portfolio coordinates, operational health readings, tree histories, and Ganoderma indicators remain simulated deterministic demo values. Mapped POC source-backed coordinates are DJI camera exposure positions; Area 003 explicitly reuses Area 001 capture evidence, all fixed records are user-designated Infected, and optional marker statuses use deterministic score bands. None is a surveyed palm-base position or confirmed diagnosis.
