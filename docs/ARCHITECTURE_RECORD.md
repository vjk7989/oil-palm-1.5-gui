...
- `MPOC-SURVEY-001` always activates 27 immutable source-backed records from `DJI_202606200943_001_DJI-SmartFarm-Web` and may activate zero through 37 saved added records from the reserved `TREE-0113...TREE-0149` range. It therefore contains 27 through 64 active trees. Every active Area 001 tree is **Infected** with a deterministic modelled score from 66 through 95. Only the 27 source records retain capture metadata and a linked source photograph; added records receive their exact operational coordinates from browser-local marker placement and have no source image.
...
Active Tree Detail routes omit multi-image galleries. Mapped POC Tree Detail shows a single linked source photograph only for the 27 immutable source records; added Area 001 trees have no source image. Mapped POC details do not render a per-tree camera-position map, and the stale Leaflet tree-detail initialization path was removed with that surface. Every active added tree nevertheless displays its exact saved latitude and longitude with its deterministic Ganoderma indicator, record metadata, history, recommendation, and status interpretation. This presentation does not alter survey-area maps, geofences, marker editing, selection, or exact-tree navigation.

Manual acceptance on 2026-09-11 verified live Satellite loading, inside-geofence add, immutable-pin rejection, click-to-remove for an added marker, undo, atomic save, synchronized 28-tree map and grid, exact added-tree coordinates without an image, a source-tree image, reload persistence, and restoration of the clean 27-tree state.

Final independent `npm test`, `npm run build`, and `git diff --check` gates were green for the dynamic marker-to-tree workflow. Automated verification covered version-5 migration, zero through 37 optional records, lowest-ID reuse, add/remove/undo/cancel/save behavior, storage rollback, geofence and source-pin collision protection, 27-through-64 synchronized map/grid/detail identities, exact added-tree coordinates and indicators, source-only imagery, no per-tree map, unchanged Areas 002-005, and continued tree-grid access when Google Maps is unavailable.
