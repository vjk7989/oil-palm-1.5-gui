...
- The combined display-status totals are 27 infected, 20 suspected, and 90 healthy. The original workbook classification and modelled score remain separately inspectable rather than being rewritten to manufacture the user-designated infection state.
...
The editor uses the native `google.maps.Rectangle` with `editable` and `draggable` enabled only during explicit edit mode. It does not load or depend on the discontinued Google Maps Drawing Library. The shared browser-local document is version `3` and adds `geofenceOverrides`, keyed by `AREA_ONE_ID`; migration preserves prior valid demo state. Only a finite, non-zero rectangle with `south < north`, `west < east`, and coordinates inside the configured Andhra Pradesh limits is accepted. Invalid or unavailable storage leaves the deterministic or last valid in-memory boundary authoritative.
...
On 2026-09-10, the temporary key successfully loaded the live Google Satellite map for Area 001. Independent `npm test`, `npm run build`, and `git diff --check` gates were green. Verification covered lazy Area 001-only loading, all-role edit access, the exact default bounds, 27 infected pins, live coordinate updates, save/cancel/reset, version-3 migration and reload persistence, invalid-bound rejection, specific load-error fallbacks, unchanged Leaflet behavior for Areas 002-005, and continued tree-grid access when Google Maps is unavailable.
