...
- The active portfolio therefore ranges from 112 through 149 trees: 27 through 64 infected, 20 suspected, and 65 healthy.
...
Only `MPOC-SURVEY-001` uses the Google Maps JavaScript API. The integration loads lazily when Area 001 opens, selects Satellite imagery, and renders the 27 immutable source positions plus any saved added positions. Areas 002-005 retain their existing Leaflet maps and deterministic camera-footprint hulls; the Google integration does not change their data or rendering path. Map failure is non-blocking, so Area 001's 8x8 tree grid and record navigation remain available.
...
Area 001 adds one native select control containing every active infected tree, source-backed or added. Selection state is deliberately transient and stores the exact Tree ID only; it is not added to browser-local persistence or the data snapshot. Selecting a tree pans and zooms the Google map to its exact coordinate. The corresponding marker becomes enlarged, outlined, and check-marked, while adjacent text identifies the selected Tree ID so selection is not conveyed by colour or marker shape alone. **Open selected tree** and direct marker clicks both resolve through that same identity and open the matching tree record. Areas 002-005 receive no selector or selection-state changes. The intentionally small scope adds no search, grouping, bulk selection, or new persistence contract.

## Decision: dynamic Survey Area 001 marker-to-tree workflow

**Add marker** is available only after the user explicitly saves a valid Area 001 geofence. Marker editing starts from the saved optional subset and allows zero through 37 added positions. A map click assigns the lowest available ID from `TREE-0113...TREE-0149`; an added marker can be removed by exact ID, and undo reverses the most recent add or remove. Cancel discards the complete draft. Save validates and atomically persists the complete resulting subset in the version-5 browser-local document; a failed write restores the previous state rather than exposing a partial update.

Every saved optional coordinate must be finite, inside the effective geofence, unique within the optional set, and different from all 27 immutable source coordinates. A later geofence save or reset is rejected when the proposed bounds would exclude a saved position. After save, one synchronized active collection drives the Area 001 metrics, numbered map pins, 8x8 grid, selector, and Tree Detail route, so a Tree ID and its exact coordinate cannot diverge across surfaces. Areas 002-005 do not read or mutate this state. The concise implementation and invariants are pinned in [`index.html`](../index.html) and [`tests/static-check.mjs`](../tests/static-check.mjs); no arbitrary tree IDs or backend contract is introduced.

## Decision: simplified active Tree Detail surfaces

Active Tree Detail routes omit multi-image galleries. Mapped POC Tree Detail shows one image: each immutable source record retains its one-to-one source photograph, while an added Area 001 record uses the geographically nearest eligible context image documented below. Mapped POC details do not render a per-tree camera-position map, and the stale Leaflet tree-detail initialization path was removed with that surface. Every active added tree displays its exact saved latitude and longitude with its deterministic Ganoderma indicator, record metadata, history, recommendation, and status interpretation. This presentation does not alter survey-area maps, geofences, marker editing, selection, or exact-tree navigation.

## Decision: nearest eligible image for added Area 001 trees

The read-only `D:\hio01` inventory contains 19 eligible Area 001 natural-colour captures corresponding to `TREE-0001...TREE-0019`. No inventory file is copied and the browser has no runtime dependency on that path; the UI reuses the matching repository-owned WebP derivatives already present for those source records. An added Area 001 tree selects exactly one eligible image by shortest geographic distance from its saved coordinate, with ascending Tree ID as the deterministic tie-break. Source-backed trees retain their exact one-to-one image association, and Areas 002-005 are unchanged. Added-tree detail explicitly discloses that the selected photograph is a nearby context image and does not depict the exact added tree.

Manual acceptance on 2026-09-11 placed `TREE-0113` at `16.912656512, 81.169811228`. The 28-tree grid and map stayed synchronized; Tree Detail showed its exact coordinate, 77% infected indicator, and nearest `TREE-0015` context image at approximately 5 m with the non-exact-tree disclosure. Source-tree one-to-one imagery also remained intact. The test marker was then removed and the clean 27-tree state restored.

Final independent `npm test`, `npm run build`, and `git diff --check` gates were green. Automated verification covered version-5 migration, zero through 37 optional records, lowest-ID reuse, add/remove/undo/cancel/save behavior, storage rollback, geofence and source-pin collision protection, 27-through-64 synchronized map/grid/detail identities, exact added-tree coordinates and indicators, deterministic nearest-image selection and disclosure, no per-tree map, unchanged Areas 002-005, and continued tree-grid access when Google Maps is unavailable.
