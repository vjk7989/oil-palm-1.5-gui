...
[`scripts/build_mapped_poc_snapshot.py`](../scripts/build_mapped_poc_snapshot.py) is the deterministic ingestion and derivation boundary. The current generated runtime contains exactly `MPOC-SURVEY-001`, `MPOC-SURVEY-002`, and `MPOC-SURVEY-003`, in that order. The two retired survey areas have no runtime data, route, map/list entry, provenance row, or generated record. Their 20 repository WebP derivatives were deleted; external source inventories remain untouched.

The active Mapped POC model is intentionally small and explicit:

- `MPOC-SURVEY-001` activates 27 immutable source-backed **Infected** records and zero through 37 optional records from `TREE-0113...TREE-0149`.
- `MPOC-SURVEY-002` activates 35 immutable source-backed **Infected** records and zero through 29 optional records from `TREE-0172...TREE-0200`.
- `MPOC-SURVEY-003` contains 64 inactive **layout-only** capacity records from `TREE-0201...TREE-0264`; it has no fixed or initially active trees.

The fresh portfolio therefore has 62 fixed Infected trees across Areas 001 and 002; Area 003 starts at zero. The maximum remains 192 active trees when all marker capacity is filled. Each area renders at most 64 active records in its existing 8x8 layout, with unused cells black and disabled. Marker-created Red/Infected, Yellow/Suspected, and Green/Healthy records receive deterministic modelled scores in the corresponding 66-95, 35-65, and 10-34 bands. Marker-selected status is user-designated and is not a field or laboratory diagnosis.

The snapshot is the immutable data and capacity source. Versioned browser-local state supplies only independently saved geofences and marker positions. Area 003 capacity records contain stable IDs and null capture, coordinate, score, status, and evidence fields until a saved marker activates them. The browser performs no runtime reads from source drives. Source inputs remain external and read-only; only optimized, repository-owned evidence files are served.

Source coordinates are DJI camera exposure positions, not surveyed palm-base locations. Added-marker coordinates and saved rectangles/polygons are operational display coordinates and boundaries, not cadastral, ownership, acreage, or legal evidence. Mapped POC Tree Detail does not render a per-tree position map and does not invent a trend from a single modelled score. Long identifiers, mission names, metadata, legends, and responsive detail columns retain explicit wrapping and shrinkable grid tracks.

## Decision: Survey Areas 001 and 003 Google Satellite rectangle editors

Survey Areas 001 and 003 use independent Google Maps rectangle editors at the same geographic location; Area 002 uses the polygon editor documented below. Each integration loads lazily and remains non-blocking so its tree selector, 8x8 grid, and record navigation survive map failure.
...
Both rectangle editors use native `google.maps.Rectangle` instances and do not load the discontinued Drawing Library. A locked rectangle is `clickable: false`; explicit geofence edit mode toggles `clickable`, `editable`, and `draggable` together, then disables all three when editing ends. Only a finite, non-zero rectangle with `south < north`, `west < east`, coordinates inside the supported extent, and containment of every saved marker is accepted. Save is atomic, Cancel restores the previous effective bounds, and Reset restores the area default only when it would not exclude saved markers.

The shared browser-local document is version `9`. Migration preserves valid Area 001 and Area 002 geofence/marker state without reinterpretation, preserves a valid Area 003 geofence override, and deliberately clears the old Area 003 marker collection so retired copied or previously saved trees cannot survive the transition. Area 003 then starts with an empty `areaThreeTreePositions` branch. Every editor reads and writes only its own state branch. Invalid or unavailable storage leaves the deterministic or last valid in-memory data authoritative.
...
Each survey has one native selector containing every active tree. Selection remains transient and stores only the exact Tree ID. Selecting a tree pans and zooms the map to its coordinate, applies the accessible selected-marker treatment, and exposes the same Tree Detail route used by a direct marker or grid-cell click. The intentionally small scope adds no search, grouping, bulk selection, or additional persistence contract.

## Decision: three-colour marker-to-tree workflow

**Add marker** is available only after the user explicitly saves a valid geofence. A three-choice toolbar selects red/Infected, yellow/Suspected, or green/Healthy before placement. Marker editing starts from the saved optional subset. A map click assigns the lowest available ID in that area's reserved range with the selected status; an added marker can be removed by exact ID, and Undo restores the complete marker, including status. Cancel discards the draft. Save validates and atomically persists the complete optional subset; a failed write restores the previous state rather than exposing a partial update.

Every saved optional marker must have one canonical status and a finite coordinate inside its effective geofence, unique within its optional set, and different from fixed coordinates. Unknown statuses, invalid coordinates, duplicates, out-of-bound points, or capacity overflow fail closed. One synchronized active collection per area drives marker colour, grid colour, selector copy, Tree Detail status/score, and survey/portfolio totals so identity, coordinate, and status cannot diverge across surfaces. The concise invariants are pinned in [`index.html`](../index.html) and [`tests/static-check.mjs`](../tests/static-check.mjs); no arbitrary IDs or backend contract is introduced.

## Decision: simplified active Tree Detail surfaces

Active Tree Detail routes omit multi-image galleries. Mapped POC Tree Detail shows exactly one allowed image for Infected/red and Suspected/yellow records under the area rules, while Healthy/green details omit the complete image/evidence section and image-loading bindings. Details do not render a per-tree camera-position map. Every active added tree still displays its exact saved latitude and longitude with its deterministic Ganoderma indicator, record metadata, history, recommendation, and status interpretation.

## Decision: allowlisted image resolution for flagged Area 001 trees

The read-only `D:\hio01` inventory establishes the repository allowlist of 19 eligible Area 001 natural-colour captures, `TREE-0001...TREE-0019`. No inventory file is copied and the browser has no runtime dependency on that path; the UI reuses only matching repository-owned WebP derivatives. Exact source records retain their one-to-one images. Other fixed or added Infected/Suspected records select one allowed image by shortest geographic distance, with ascending Tree ID as the deterministic tie-break, and disclose when it is nearby context rather than the exact tree. Healthy added trees do not invoke this resolver.

## Decision: Survey Area 003 empty marker workspace

Area 003 is an independent empty workspace that references Area 001 only for its operational location. Its snapshot declares `locationReferenceAreaId: "MPOC-SURVEY-001"`; it does not copy captures, capture UUIDs, fixed trees, statuses, scores, or evidence records. All 64 globally unique IDs `TREE-0201...TREE-0264` are inactive `layout-only` capacity records until corresponding browser-local marker positions are saved.

Its deterministic operational rectangle has southwest `16.912209794, 81.169804433` and northeast `16.912805551, 81.169912437`, represented by the four corners `(16.912209794, 81.169804433)`, `(16.912209794, 81.169912437)`, `(16.912805551, 81.169912437)`, and `(16.912805551, 81.169804433)`. The rectangle is an editable display boundary, not capture evidence or a surveyed legal boundary.

Area 003 has its own saved rectangle, marker collection, editing mode, selected tree, and transient draft. Saving markers activates exactly the corresponding layout-only IDs; saving zero markers is valid and produces no pins, an empty selector, and a fully black 8x8 grid. IDs are assigned and reused from the lowest available value. Add/remove/undo/cancel/atomic-save behavior and the Red/Infected, Yellow/Suspected, and Green/Healthy score bands match the other editors without reading or mutating Area 001 or Area 002 state.

Area 003 Infected or Suspected markers resolve the nearest repository-owned Area 001 image and retain the non-exact-tree disclosure; Healthy markers show no image. This change required no image deletion because the retired Area 003 fixed records referenced shared Area 001 assets rather than owning duplicates.

Because Areas 001 and 003 have identical centroids, Overview renders one combined map pin labelled for both surveys instead of overlapping pins. Its popup presents separate actions that open the exact requested survey. The accessible list and Table view keep separate entries, so the combined map geometry does not collapse their identities or state.
...
The Area 001 version-6 independent `npm test`, `npm run build`, and `git diff --check` gates were green, and manual browser verification passed. Coverage included version-5 migration, the three canonical status/score bands, marker editing and validation, synchronized map/grid/selector/detail/totals, flagged-tree allowlisted-image selection, Healthy evidence omission and no image binding, no per-tree map, then-unchanged surrounding survey workflows, and continued tree-grid access when Google Maps is unavailable.

## Decision: Survey Area 002 `frm02` source integration

This decision supersedes the earlier Area 002 snapshot count while leaving the independent Area 001 and Area 003 workspaces unchanged. The deterministic build-time inventory reads `D:\frm02` as a read-only source and selects 35 unique natural-colour captures numbered `0002...0036`. XMP supplies each camera exposure coordinate, and each record keeps an exact one-to-one optimized WebP derived from its matching `D.JPG`. No source file is copied, and the browser has no runtime dependency on `D:\frm02`.

The stable records use the existing `TREE-0028...TREE-0040` IDs plus `TREE-0150...TREE-0171`, avoiding any renumbering of the other survey areas. All 35 source-backed records are user-designated **Infected**, with `healthStatus` **Unhealthy**, `severity` **Severe**, and deterministic modelled Ganoderma scores from 66 through 95. Their source mission is `DJI_202606201109_004_DJI-SmartFarm-Web`. [`scripts/build_survey_two_farm_snapshot.py`](../scripts/build_survey_two_farm_snapshot.py) is the authoritative derivation boundary and regenerated the inspectable [`data/survey-002-farm-data.js`](../data/survey-002-farm-data.js) snapshot consumed by the existing UI. The regeneration changes the classification fields only; IDs, XMP coordinates, exact source-image associations and checksums, and the polygon geofence remain unchanged.

The `D:\frm02` inventory contains 35 complete DJI acquisitions; their 35 `D.JPG` observations are already represented by the repository snapshot and optimized assets, so the browser performs no runtime source-directory read. Area 002 uses Google Satellite with its 13-point camera-footprint hull as the default native `google.maps.Polygon`. Explicit edit mode enables vertex editing and whole-polygon dragging. Save accepts only a finite, non-self-intersecting polygon inside the supported AP extent that contains all 35 immutable source coordinates and every saved added marker; Cancel restores the effective polygon, and Reset refuses a default that would exclude saved trees.

Area 002 keeps its 35 red/Infected source trees immutable and may persist zero through 29 additions using `TREE-0172...TREE-0200`, filling the shared 64-cell capacity without altering existing IDs. Added markers persist **Infected**, **Suspected**, or **Healthy** status with deterministic scores in the 66-95, 35-65, or 10-34 bands. Invalid status or coordinates, duplicate/source-overlapping positions, out-of-polygon points, and excess capacity fail closed; add/remove/undo/cancel operate on a detached draft and Save replaces the optional collection atomically. One synchronized active collection drives polygon/map marker colour and numbering, the selector, grid, Tree Detail identity/status, and totals.

Each of the 35 source trees retains its exact one-to-one repository image. Added Infected and Suspected trees select the deterministic nearest Area 002 source image with a non-exact-tree disclosure; added Healthy trees render no image or loading binding. Google load failures retain the specific diagnostic fallback and usable grid. Area 001 and Area 003 rectangle, marker, and image rules remain isolated from this polygon workflow.

Manual browser acceptance verified Satellite loading, 13-point polygon editing and persistence, containment guards, fixed and added marker interaction, synchronized selector/grid/detail/totals, and unchanged Area 001 behavior. Independent `npm test`, `npm run build`, and `git diff --check` gates were green.
...
- AP portfolio coordinates, operational health readings, tree histories, and Ganoderma indicators remain simulated deterministic demo values. Mapped POC source-backed coordinates are DJI camera exposure positions; Area 003 references Area 001 only as its operational location and activates trees solely from browser-saved markers. Fixed Area 001 and Area 002 records are user-designated Infected, while marker statuses use deterministic score bands. None is a surveyed palm-base position or confirmed diagnosis.
