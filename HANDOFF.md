...
- Survey Area 002 now has a verified editable Google Satellite polygon and three-colour marker-to-tree workflow built on its deterministic 35-observation `frm02` snapshot.
...
- Area 002 starts with 35 immutable healthy source observations and exact repository images from the `D.JPG` inventory under read-only `D:\frm02`. Its polygon can be explicitly edited, saved, cancelled, or reset, while validation prevents invalid/self-intersecting shapes or exclusion of fixed and saved markers.
...
- Survey Area 002 automated and manual verification is green for polygon edit/save/cancel/reset, three-colour add/remove/undo/save/reload, stable marker/grid/selector/detail identity, dynamic counts, exact and nearest/no-image behavior, Google failure fallback, version-7 migration, and strict isolation from Area 001 and Areas 003–005. See `docs/ARCHITECTURE_RECORD.md` and `tests/static-check.mjs` for the detailed acceptance record.
...
4. Commit the Area 002 version-7 workflow, push `main`, and verify the resulting GitHub Pages build live before calling the increment released.
